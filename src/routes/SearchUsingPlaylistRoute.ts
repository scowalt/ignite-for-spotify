...
import { PlaylistItem, Paging, SimplifiedArtist } from 'spotify-web-api-ts/types/types/SpotifyObjects';
	...
	do {
		...
		for (const value of playlistTracks.items) {
			...
			if (newTracks.length > 10) {
				Logger.getInstance().warn(`More than ten results for track ${JSON.stringify(basicTrack)}`);
			}
